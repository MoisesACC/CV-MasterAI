...
import { CheckCircle, AlertTriangle, XCircle, ChevronRight, Wand2 } from 'lucide-react';
  ...
  return (
    <div className="relative h-48 w-48 mx-auto">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pt-8">
        ...
        <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">ATS Score</span>
      </div>
    </div>
  );
};
...
const SectionCard: React.FC<{ title: string; data: SectionAnalysis }> = ({ title, data }) => {
  const getIcon = () => {
    switch (data.status) {
      case 'good': return <CheckCircle className="text-green-500 dark:text-green-400" size={20} />;
      case 'warning': return <AlertTriangle className="text-amber-500 dark:text-amber-400" size={20} />;
      case 'critical': return <XCircle className="text-red-500 dark:text-red-400" size={20} />;
    }
  };

  const getBgColor = () => {
    switch (data.status) {
      case 'good': return 'bg-green-50 border-green-100 dark:bg-green-900/10 dark:border-green-900/30';
    ...
    <div className={`p-4 rounded-lg border ${getBgColor()} transition-colors`}>
      ...
      <div className="space-y-2">
        {data.feedback.map((item, idx) => (
          <p key={idx} className="text-sm text-slate-600 dark:text-slate-300 flex items-start gap-2">
            <span className="mt-1.5 w-1 h-1 rounded-full bg-slate-400 dark:bg-slate-500 flex-shrink-0" />
            {item}
          </p>
        ))}
      </div>
      <div className="mt-3 text-xs font-bold text-slate-500 dark:text-slate-400 text-right">
...
export const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ analysis, onOptimize }) => {
  return (
    <div className="w-full max-w-5xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      {/* Header Summary */}
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-8 items-center transition-colors">
        <div className="flex-shrink-0">
          <ScoreGauge score={analysis.overallScore} />
        </div>
        <div className="flex-grow space-y-4">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Diagnóstico General</h2>
          <p className="text-slate-600 dark:text-slate-300 leading-relaxed">{analysis.summary}</p>
          
          <div className="flex flex-wrap gap-3">
             <div className={`px-3 py-1 rounded-full text-xs font-medium border
                ${analysis.formatting.isClean 
                  ? 'bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800' 
                  : 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800'}`}>
                Formato {analysis.formatting.isClean ? 'Limpio' : 'Problemático'}
             </div>
             <div className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-4 rounded-lg border bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 transition-colors">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-slate-800 dark:text-slate-100">Palabras Clave ATS</h3>
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Gap Analysis</span>
          </div>
          <div className="space-y-4">
            <div>
              <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1">Encontradas:</p>
              <div className="flex flex-wrap gap-1">
                {analysis.atsKeywords.found.map(k => (
                  <span key={k} className="px-2 py-0.5 bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs rounded">{k}</span>
                ))}
                {analysis.atsKeywords.found.length === 0 && <span className="text-xs text-gray-400">Ninguna relevante detectada</span>}
              </div>
            </div>
            <div>
              <p className="text-xs text-red-600 dark:text-red-400 font-semibold mb-1">Faltantes (Crítico):</p>
              <div className="flex flex-wrap gap-1">
                {analysis.atsKeywords.missing.map(k => (
                  <span key={k} className="px-2 py-0.5 bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 text-xs rounded">{k}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-900 dark:bg-slate-800 text-white rounded-xl p-6 md:p-8 shadow-lg transition-colors">
        ...
        <ul className="space-y-3 mb-8">
          {analysis.recommendations.map((rec, i) => (
            <li key={i} className="flex items-start gap-3">
              ...
              <span className="text-slate-200 dark:text-slate-300">{rec}</span>
            </li>
          ))}
        </ul>
